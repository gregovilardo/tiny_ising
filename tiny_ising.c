...
static void cycle(int grid[L][L / NBITS], const double min, const double max,
...
static void init(int grid[L][L / NBITS]) {
  for (unsigned int i = 0; i < L; ++i) {
    for (unsigned int j = 0; j < L / NBITS; ++j) {
      grid[i][j] = -1;
    }
  }
}
