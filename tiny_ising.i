...
void update(const float temp, int grid[1024][1024]);
double calculate(int grid[1024][1024], int *M_max);
# 13 "tiny_ising.c" 2
...
# 135 "/usr/include/bits/mman-linux.h" 3 4
# 1 "/usr/include/bits/mman-shared.h" 1 3 4
# 136 "/usr/include/bits/mman-linux.h" 2 3 4
...
static void cycle(int grid[1024][1024], const double min, const double max,
                  const double step, const unsigned int calc_step,
                  struct statpoint stats[]) {

  (((0 < step && min <= max) || (step < 0 && max <= min)) ? (void) (0) : __assert_fail ("(0 < step && min <= max) || (step < 0 && max <= min)", "tiny_ising.c", 49, __extension__ __PRETTY_FUNCTION__));
  ...
  for (double temp = min; modifier * temp <= modifier * max; temp += step) {
    ...
    ((index < (1 + (int)((3.0 - 1.5) / 0.01))) ? (void) (0) : __assert_fail ("index < NPOINTS", "tiny_ising.c", 85, __extension__ __PRETTY_FUNCTION__));
    stats[index].t = temp;
...
static void init(int grid[1024][1024]) {
...
int main(void) {
  ...
  _Static_assert(0.01 != 0, "Invalid temperature step");
  _Static_assert(((0.01 > 0) && (1.5 <= 3.0)) ||
                    ((0.01 < 0) && (1.5 >= 3.0)),
                "Invalid temperature range+step");
  _Static_assert(
      25 % 5 == 0,
      "Measurements must be equidistant");
  _Static_assert(
      (1024 * 1024 / 2) * 4ULL < (2147483647 *2U +1U),
  ...
  int(*grid)[1024] = mmap(((void*)0), 1024 * 1024 * sizeof(int), 0x1 | 0x2,
                       0x02 | 0x20 | 0x40000, -1, 0);
  ...
  for (unsigned int i = 0; i < (1 + (int)((3.0 - 1.5) / 0.01)); ++i) {
    printf("%lf,%.10lf,%.10lf,%.10lf,%.10lf,%.10lf,%.10lf\n", stat[i].t,
           stat[i].e / ((double)(1024 * 1024)), stat[i].e2 / ((double)(1024 * 1024) * (1024 * 1024)),
           stat[i].e4 / ((double)(1024 * 1024) * (1024 * 1024) * (1024 * 1024) * (1024 * 1024)), stat[i].m, stat[i].m2,
           stat[i].m4);
  }
  printf("=====================\n");
  float cells_ns = (1024 * 1024 * nupdates) / (duration * 1e9);
  printf("cells/ns %f\n", cells_ns);
  ...
  munmap(grid, 1024 * 1024 * sizeof(int));
