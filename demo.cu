...
static void draw(gl2d_t gl2d, float t_now, float t_min, float t_max,
  ...
  for (int i = 1; i < 1 + L; ++i) {
    ...
    for (int j = 1; j < 1 + L; ++j) {
      int cell = (i % 2 == 0) ? black_grid[(i / 2) * COLS + j]
                              : red_grid[(i / 2) * COLS + j];
      if (cell > 0) {
        row[j * 3] = color[0];
        row[j * 3 + 1] = color[1];
...
static void init(int *grid) {
  for (size_t i = 1; i < 1 + L / 2; ++i) {
    for (size_t j = 1; j < 1 + L; ++j) {
      grid[i * COLS + j] = (rand() / (float)RAND_MAX) < 0.5f ? -1 : 1;
    }
  }

  // Duplicate first row to last row, and last to first
  for (size_t j = 0; j < COLS; ++j) {
    grid[0 * COLS + j] = grid[(L / 2) * COLS + j];
    grid[(1 + L / 2) * COLS + j] = grid[1 * COLS + j];
  }

  // Duplicate first column to last, and last to first
  for (size_t i = 0; i < 2 + L / 2; ++i) {
    grid[i * COLS + 0] = grid[i * COLS + L];
    grid[i * COLS + (1 + L)] = grid[i * COLS + 1];
  }
}
...
int main(void) {
  ...
  static_assert((L * L / 2) * 4ULL < UINT_MAX, "L too large for uint indices");
  ...
  int *black_grid, *red_grid;
  cudaMallocManaged(&black_grid, size);
  cudaMallocManaged(&red_grid, size);
